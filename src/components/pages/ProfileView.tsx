...
import { User, Sparkles, Terminal, Activity, Calendar, ShieldCheck, Mail, ShieldAlert } from 'lucide-react';

export default function ProfileView() {
  const { user } = useAuthStore();

  const timeline = [
    { action: 'Dynamic POST api called', desc: 'Processed Inventory payload entry.', time: '10m ago', type: 'api' },
    { action: 'Zod config validation', desc: 'Validated Leads funnel successfully.', time: '1h ago', type: 'validation' },
    { action: 'Project compiled', desc: 'CRM Dashboard built and deployed.', time: '1d ago', type: 'build' }
  ];
    ...
    <div className="flex flex-col gap-8 w-full max-w-4xl">
      ...
      <div className="flex flex-col gap-1 border-b border-zinc-800/60 pb-5 shrink-0">
        <h2 className="text-xl md:text-2xl font-bold tracking-tight text-zinc-100 flex items-center gap-2">
          Developer Workspace Profile <User className="w-5 h-5 text-zinc-400" />
        </h2>
          ...
          <div className="md:col-span-1 glass-panel p-6 rounded-3xl glow-violet/5 flex flex-col items-center text-center gap-4 relative overflow-hidden">
            ...
            <div className="flex flex-col gap-3 w-full text-xs text-zinc-400 pl-1">
              <div className="flex items-center gap-2">
                <Mail className="w-4 h-4 text-zinc-650 shrink-0" />
